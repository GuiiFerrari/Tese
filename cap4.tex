...
\par A normalização deve ser obtida a partir da análise do canal elástico do experimento. Até o momento da escrita dessa dissertação, ainda está sendo trabalhado o valor da normalização.

\par A energia do feixe também foi discretizada, pois existe uma incerteza associada à posição no eixo $z$ do vértice de reação. Como a energia do feixe depende da posição em $z$ do vértice de reação, ela foi discretizada a cada 10 mm, o que resulta em bins de aproximadamente 1.2 MeV de largura. A partir dessas bins, foi calculada a energia do feixe. A figura \ref{fig:contagem_energia} mostra o espectro da seção de choque em função da energia no referencial do centro de massa. As distribuições angulares estão mostradas na figura \ref{fig:dist_ang}.
...
\par Os resultados mostrados na fig \ref{fig:dist_ang} mostram distribuições com energias acima (figuras \ref{subfig:dist_ang_a} - \ref{subfig:dist_ang_g}), próximas (figuras \ref{subfig:dist_ang_h} - \ref{subfig:dist_ang_l}) e abaixo (figuras \ref{subfig:dist_ang_m} - \ref{subfig:dist_ang_ab}) da barreira de Coulomb da reação (barreira de Coulomb é de 4.2 MeV). As distribuições entre 3.63 MeV e 4.77 MeV se distribuem de modo horizontal em ângulos entre 20º e 50º. Distribuições acima da barreira possuem valores maiores em ângulos mais dianteiros (de 20º à 30º). Embaixo da barreira as distribuições são mais estreitas e parecem ter um valor máximo em 30º.
...
\par Os resultados dos cálculos de CDCC nos dados experimentais apresentados, mostram que existem interferências destrutivas e construtivas nas contribuições de Coulomb e nuclear do breakup. A contribuição parece ter uma relevância bem maior em núcleos com $Z$ menor. Em núcleos com $Z$ grande, como o caso do $^{208}$Pb, os cálculos teóricos indicam que há interação entre as contribuições, diminuindo a seção de choque \cite{Kolata2016, MORO_BREAKUP}. Para o cara de energias próximas da barreira de Coulomb, a seção de choque é ainda menor, corroborando com os dados obtidos com o experimento desse trabalho. Com esses resultados, será possível entender melhor a natureza do breakup, qual relevância de cada contribuição, como a interações entre as contribuições acontecem, em diversas regiões de energia (acima, próxima e abaixo da barreira de Coulomb).
