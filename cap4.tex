...
\par Com o alvo ativo é possível medir reações em diversos ângulos e energias. Portanto, é preciso primeiro discretizar (colocar em canais) os ângulos de espalhamento $\theta_p$ (ângulos de espalhamento dos prótons). Cada canal $\theta_{bin}$ de $\theta_p$ possui largura de 4º. A seção de choque diferencial $\frac{\mathrm{d}\sigma}{\mathrm{d}\Omega} (\theta)$ é dada por

\begin{equation}
	\frac{\mathrm{d}\sigma}{\mathrm{d}\Omega} (\theta) = \frac{1}{\Omega(\theta_p) L},
\end{equation}

\par onde $L$ é a luminosidade (fator de normalização) e $\Omega(\theta_p)$ é o ângulo sólido calculado em $\theta_p$ dado por

\begin{equation}
	\Omega (\theta_p) = 2\pi \int_{\theta_p}^{\theta_p + \theta_{bin}}\sin(\theta)\mathrm{d}\theta.
\end{equation}

\par A energia do feixe $E$ também foi divida em canais 

\par A normalização deve ser feita a partir

\par Algumas das distribuições angulares estão mostradas na figura \ref{fig:dist_ang}.

\chapter{Conclusão}\label{chapter:conclusao}

\par Este trabalho abordou o uso de técnicas de machine learning para análise de reações nucleares no alvo ativo pAT-TPC, especificamente a reação de breakup $^4$He($^{17}$F,$^{16}$O+p)$^4$He. Foi descrita a parte experimental, desde a produção do feixe $^{17}$F até o uso do alvo ativo.
