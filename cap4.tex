...
\par Os resultados das figuras \ref{fig:dist_ang_acima}, \ref{fig:dist_ang_prox} e \ref{fig:dist_ang_abaixo} mostram distribuições com energias acima (figura \ref{fig:dist_ang_acima}), próximas (figura \ref{fig:dist_ang_prox}) e abaixo (figura \ref{fig:dist_ang_abaixo}) da barreira de Coulomb da reação (4.2 MeV). As distribuições entre 3.63 MeV e 4.77 MeV se distribuem de modo horizontal em ângulos entre 20º e 50º. Distribuições acima da barreira possuem valores maiores em ângulos mais dianteiros (de 20º à 30º). Embaixo da barreira as distribuições são mais estreitas e parecem ter um valor máximo em 30º.
...
\par Este trabalho abordou o estudo da reação de breakup $^4$He($^{17}$F,$^{16}$O+p)$^4$He. A reação foi medida usando o pAT-TPC (localizado na University of Notre Dame, Estados Unidos), que é uma câmara de projeção no tempo que possui o alvo ativo gasoso $^4$He, que funciona tanto como alvo, quanto meio detector de reações nucleares e dos produtos das reações. O feixe de $^{17}$F foi produzido pelo sistema TWINSOL e incide no alvo ativo $^4$He com energia de 34.76 MeV.

\par Com o plano detector micromegas, foi possível fazer a reconstrução tridimensional dos eventos (reações nucleares), gerando milhões de nuvens de pontos 3D. Para reconstruir as nuvens de pontos, foi necessário analisar os pulsos gerados pelo micromegas, que são da ordem das centenas de milhões de sinais. Os dados possuem grande complexidade e sua análise e foi dividida em três etapas: remoção do sinal de fundo, deconvolução do sinal e identificação de picos. Para cada etapa, foi construído um banco de dados com os dados de entrada e os dados de saída (que consistem em sinais após a realização de cada etapa), com o objetivo de criar redes neurais supervisionadas que aprendessem com o padrão dos dados. Três redes neurais foram construídas e o tempo de processamento, em comparação com algoritmos mais tradicionais, diminui em cerca de 90 vezes, obtendo resultados muito similares.

\par Usando as redes neurais construídas para analisar os pulsos, as nuvens de pontos foram reconstruídas. Com as nuvens de pontos, foi necessário identificar aglomerados que formam trajetórias retas, que correspondem às trajetórias das partículas carregas ao atravessar o gás. Isso é feito através de algoritmos de clustering (machine learning não supervisionado), mas foi usado principalmente o estimador robusto pRANSAC (algoritmo \ref{ransac_algo}, descrito no capítulo \ref{chapter:point_cloud_analysis}), que é capaz de identificar aglomerados mesmo na presença de outliers. Algumas alternativas para analisar as nuvens de pontos foram estudados, como o PointNet. A rede neural PointNet pode ser usada tanto para a classificação global da nuvem de pontos, quanto para a segmentação semântica (classificação ponto a ponto). A rede foi testada para identificar quantas trajetórias existem nas nuvens de pontos e demonstrou bons resultados (boa acurácia). Também para a PointNet, a segmentação semântica foi utilizada para a identificação de outliers (globais e locais), e também demonstrou resultados satisfatórios. Para uma abordagem mais direta para o problema, como o clustering usando algoritmos de machine supervisionados, foi sugerido o uso da rede neural ConstrastNet para futuros trabalhos. A rede é capaz de fazer o clustering nos dados, retornando os aglomerados presentes nas nuvens de pontos.
...
\par Por fim, os resultados foram comparados com experimentos realizados em cinemática direta. Os experimentos mediram o breakup do $^{17}$F em $^{58}$Ni e $^{208}$Pb, com medidas próximas e acima das respectivas barreiras de Coulomb das reações. Os resultados desse estão de acordo com os experimentos prévios, que indicaram que quanto menor o número de prótons $Z$ de um núcleo, maior a seção de choque. Também foi notado um queda de aproximadamente 20\% nas seções de choque para reações com energias no centro de massa próximas da barreira de Coulomb. Os resultados corroboram com a ideia de que a interferência entre as interações nuclear e de Coulomb reduz o valor da seção de choque. Os novos resultados irão ajudar a entender melhor o mecanismo do breakup, pois leva o problema à um caso com $Z$ = 4, muito menor que outras medidas já feitas.
