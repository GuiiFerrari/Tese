...
\par Esse capítulo irá mostrar como foram analisadas as nuvens de pontos. O objetivo desta etapa, do ponto de vista computacional, foi de detectar \textit{clusters}, que nesse caso são retas tridimensionais. Com a informação da física das trajetórias (energia, momento e  comprimento), foi possível distinguir cada reta detectada, determinar o vértice de reação entre uma partícula originada de uma reação nuclear, e o feixe e por fim determinar o momento das partículas. Após a detecção, foram selecionados os eventos que possuíam reações nucleares.
