...
\par Esse capítulo mostra como foram construídas as distribuições angulares da reação de breakup do $^{17}$F a partir das informações obtidas da análise das nuvens de pontos. As ferramentas matemáticas e computacionais foram apresentadas e desenvolvidas no capítulo \ref{chapter:point_cloud_analysis} para ser possível buscar reações nucleares.


%No capítulo \ref{chapter:point_cloud_analysis} foram desenvolvidas as partes computacionais e matemáticas. A análise segue de modo a completar a cinemática das reações nucleares para identificar reações nucleares e quais as partículas identificadas.

% No capítulo \ref{chapter:point_cloud_analysis} a parte computacional e matemática foi desenvolvida, a continuidade da análise é completar a cinemática das reações nucleares, para identificar qual a reação nuclear e quais . 

\par Na seção \ref{sec:identif_reac_nucl} está descrito como identificar o canal de reação de breakup e as partículas presentes. A seção \ref{sec:sec_choque} descreve como construir as distribuições angulares.
...
\par As subseções a seguir descrevem as etapas de identificação de reações nucleares de breakup do $^{17}$F.
...
\par Portanto, cada tipo de partícula tem um alcance diferente dentro do mesmo gás que depende da energia incidente. Por exemplo, para determinar a energia cinética do $^{17}$F antes da reação nuclear, pode se usar a distancia entre a janela do detector e o vértice da reação. Para isso, o espaço no eixo $z$ foi discretizado, a fim de calcular a energia no momento da reação. O feixe, ao entrar no gás, possui energia inicial $E_i$ = 34.76 MeV, e perde energia ao atravessar o gás. A energia de reação $E_r$ é dada por

\begin{equation}
	E_r = E_i - E_{loss},
\end{equation}

\par onde $E_{loss}$ é a energia do $^{17}$F depositada no gás que é calculada a partir da distancia $L$ ate o vértice da reação. O comprimento $L$ é transformado em energia cinética  usando uma tabela de \textit{stopping power}. Similarmente, a energia de outras  partículas (i.e. alfa), que param completamente no gás, é calculado com o alcance da partícula (relativo ao vértice) e a sua respetiva tabela de \textit{stopping power}.
...
\subsection{Identificação do canal de reação de breakup}

\par Com as informações angulares, de comprimento e de energia das trajetórias, foi possível identificar o canal de reação de breakup. Com o alvo ativo, é possível medir diversos canais de reação, não só o de breakup. Isso fica evidente ao olhar para a figura \ref{fig:comp_vs_ang} (onde já foram removidas trajetórias que correspondem ao feixe espalhado à baixos ângulos), que mostra o histograma bidimensional do comprimento (mm) em função do ângulo de espalhamento (graus) no referêncial do laboratório das trajetórias identificadas.
...
\begin{figure}[H]
    \centering
    \includegraphics[scale = 0.5, width=\columnwidth]{figs/comp_vs_ang_n_base.png}\
    \caption{Histograma de comprimento de \textit{track} no eixo $y$ e ângulo de espalhamento no eixo $x$.}
    \label{fig:comp_vs_ang}
\end{figure}

\par Pela figura \ref{fig:comp_vs_ang}, é possível dois aspectos importantes sobre os dados. O primeiro é que o canal de reação elástico e inelástico é predominante no espectro. Esse é um dos motivos pela assimetria com relação ao ângulo de espalhamento. Para o espalhamento elástico, o ângulo de espalhamento no referencial do laboratório é metade do ângulo de espalhamento no referencial do centro de massa \cite{two_body_mec}, portanto é natural haver muito mais trajetórias com ângulos de espalhamento menores que 90º no referencial do laboratório. Ângulos maiores que 90º na figura representam eventos de cinemática inversa. O segundo aspecto é a ausência de trajetórias na região central da figura. Essa ausência ocorre devido à limitações geométricas da câmara. Por exemplo, uma trajetória com ângulo de espalhamento de cerca de 90º não pode ter comprimento maior que 140 mm, pois o evento ocorre na região central da câmara e isso significa ultrapassar o limite geométrico do TPC.
...
\par É necessário aplicar vários gates (condições de inclusão os exclusão de trajetórias) para poder identificar trajetórias cujo canal de reação foi o de breakup. O primeiro gate aplicado para a identificação, como mostrado na figura \ref{fig:gate_1}, é tal que trajetórias que possuem comprimento e carga total fora de determinada região (ver linha vermelha na figura \ref{subfig:carga_comp_nocut}), foram excluídos. Além disso, para excluir trajetórias cujo vértice de reação, no eixo $z$, estavam 30 mm e 70 mm com ângulos de espalhamento entre 10º e 100º também foram removidas. A figura \ref{subfig:carga_comp_cut1} mostra o resultado da aplicação do gate. O resultado da aplicação do gate para o espectro de comprimento em função do ângulo de espalhamento (ver figura \ref{fig:comp_vs_ang}) está na figura \ref{subfig:comp_vs_carga_cut1}.

\begin{figure}[H]
\centering
    \begin{subfigure}[b]{\textwidth}
        \centering
        \includegraphics[scale = 0.5, width=\columnwidth]{figs/carga_vs_comp_nc_cut1.png}
    ...
    \begin{subfigure}[b]{\textwidth}
        \centering
        \includegraphics[scale=0.5, width=\columnwidth]{figs/carga_vs_comp_cut1.png}
...
%    \hspace{0.5cm}
    ...
    \begin{subfigure}[b]{\textwidth}
        \centering
        \includegraphics[scale=0.5, width=\columnwidth]{figs/comp_vs_ang_n_base_cut_alpha1.png}
    ...
    \end{subfigure}%
    \hfill
\caption{Processo de aplicação do primeiro gate para identificar as trajetórias do anal de reação de breakup. O resultado é o espectro da figura \ref{subfig:comp_vs_carga_cut1}.}
\label{fig:gate_1}
\end{figure}

\par O primeiro gate manteve trajetórias maiores que 60 mm e que não depositam muita energia no gás, o que removeu trajetórias de partículas alfas vindas do espalhamento elástico com o feixe.

\par O segundo gate elimina trajetórias originadas de partículas pesadas. O gate está indicado na figura \ref{subfig:gate2_antes}. O resultado, para o espectro de comprimento em função do ângulo de espalhamento, está na figura \ref{subfig:gate2_depois}, onde é possível distinguir dois tipos de trajetórias, uma originada por partículas pesadas ($^{16}$O) e uma por partículas leves (próton). O espectro indica que o próton não para no gás, pois suas trajetórias estão limitadas pela câmara, independente do ângulo de espalhamento. Isso ocorre pois o próton deposita pouca energia ao atravessar o gás. Já o $^{16}$O para completamente no gás, e possui trajetórias mais dianteiras (ângulos de espalhamento menores).
...
\centering
    \begin{subfigure}[b]{\textwidth}
        \centering
        \includegraphics[scale = 0.5, width=\columnwidth]{figs/densidade_carga_cut_den1.png}
        \caption{Histograma bidimensional da densidade de carga em função do comprimento (mm). A linha vermelha indica o segundo gate, onde pontos (trajetórias) dentro da região foram descartados.}
        \label{subfig:gate2_antes}
    \end{subfigure}%
    \hfill
    \begin{subfigure}[b]{\textwidth}
        \centering
        \includegraphics[scale=0.5, width=\columnwidth]{figs/comp_vs_ang_n_base_cut_alpha12_ident.png}
        \caption{Histograma bidimensional do comprimento (mm) em função do ângulo de espalhamento (graus) após a aplicação do gate mostrado na figura \ref{subfig:gate2_antes}. Com esse gate, é possível identificar as trajetórias do $^{16}$O e o próton, indicadas na figura.}
...
\par Para ilustrar o significado da aplicação do último gate, e o resultado mostrado na figura \ref{subfig:gate2_depois}, o evento mostrado na figura \ref{fig:exemplo_proton_16O} mostra três trajetórias. A trajetória do próton (em cor verde) está no limite geométrico da câmara (em cor cinza transparente), e com ângulo próximo de 90º. O $^{16}$O originou a trajetória de cor amarela, e possui comprimento muito menor que o do próton, além de um ângulo de espalhamento mais dianteiro.

\begin{figure}[H]
    \centering
    \includegraphics[scale = 1., width=0.75\columnwidth]{figs/evento_ex_4186.png}
    \caption{Evento onde há a trajetória originada por um próton (trajetória em verde), onde percebe-se que seu comprimento vai até o limite geométrico da câmara (cilindro cinza transparente). O feixe está em azul e o $^{16}$O originou a trajetória em amarelo.}
    \label{fig:exemplo_proton_16O}
\end{figure}

\par Identificado o canal de breakup, foi possível construir as distribuições angulares da reação de breakup.

%\par Também para remover trajetórias do feixe espalhadas, foram removidas trajetórias com ângulo de espalhamento muito baixo. O gate 3 está na figura \ref{subfig:gate3_antes} e o resultado de sua aplicação está na figura \ref{subfig:gate3_depois}, onde é possível perceber que as partículas resultantes até então são as que possuem trajetórias grandes (pois estão limitadas pela geometria da câmara, como discutido anteriormente).
%
%\begin{figure}[H]
%\centering
%    \begin{subfigure}[b]{\textwidth}
%        \centering
%        \includegraphics[scale = 0.5, width=\columnwidth]{figs/comp_vs_ang_cut12_com_cut3.png}
%        \caption{Histograma bidimensional do comprimento (mm) em função do ângulo de espalhamento (graus) no referencial do laboratório. A linha vermelha indica o gate 3, onde pontos (trajetórias) dentro da região foram descartados.}
%        \label{subfig:gate3_antes}
%    \end{subfigure}%
%    \hfill
%    \begin{subfigure}[b]{\textwidth}
%        \centering
%        \includegraphics[scale=0.5, width=\columnwidth]{figs/comp_vs_ang_cut123.png}
%        \caption{Histograma bidimensional do comprimento (mm) em função do ângulo de espalhamento (graus) após a aplicação do gate 3.}
%        \label{subfig:gate3_depois}
%    \end{subfigure}%
%%    \hspace{0.5cm}
%	\hfill
%\caption{Processo de aplicação do terceiro gate para identificar os prótons. O resultado é um espectro da figura \ref{subfig:comp_vs_carga_cut1}.}
%\label{fig:gate_3}
%\end{figure}
%
%\par Os últimos gates aplicados foram para remover possíveis espalhamentos elásticos de partículas alfa. Trajetórias cujo vértice de reação, na coordenada $z$, estavam entre 30 mm e 70 mm e ângulo de espalhamento entre 10º e 100º foram removidas. Junto ao último gate, trajetórias com ângulo de espalhamento menores que 5º ou maiores que 175º foram removidas. A figura \ref{subfig:antes_cut4} mostra o gráfico do comprimento em função do ângulo de espalhamento antes da aplicação dos últimos gates e a figura \ref{subfig:depois_cut4} mostra o mesmo gráfico após a aplicação deles.
%
%\begin{figure}[H]
%\centering
%    \begin{subfigure}[b]{\textwidth}
%        \centering
%        \includegraphics[scale = 0.5, width=\columnwidth]{figs/comp_vs_ang_cut1234.png}
%        \caption{Histograma bidimensional do comprimento (mm) em função do ângulo de espalhamento (graus) no referencial do laboratório. Trajetórias com baixo ângulo espalhamento ainda estão presentes.}
%        \label{subfig:antes_cut4}
%    \end{subfigure}%
%    \hfill
%    \begin{subfigure}[b]{\textwidth}
%        \centering
%        \includegraphics[scale=0.5, width=\columnwidth]{figs/comp_vs_ang_cut12345.png}
%        \caption{Histograma bidimensional do comprimento (mm) em função do ângulo de espalhamento (graus) após a aplicação do último gate.}
%        \label{subfig:depois_cut4}
%    \end{subfigure}%
%%    \hspace{0.5cm}
%	\hfill
%\caption{Processo de aplicação do último gate para identificar os prótons. O resultado é um espectro da figura \ref{subfig:depois_cut4}.}
%\label{fig:gate_45}
%\end{figure}

%\par Com os gates aplicados, é possível ver, pela figura \ref{subfig:depois_cut4}, que as trajetórias restantes estão todas no limite geométrico da câmara, ou seja, as partículas não param no gás, indicando que as trajetórias foram originadas por prótons (que possui baixa perda de energia no gás).
%
%\par Com os prótons identificados, foi possível criar novos gates para identificar sua presença nos eventos.



%%\par A figura \ref{fig:coincidencia_detec} mostra o histograma, onde agora é possível notar as características de trajetórias de partículas diferentes.
%%
%%\begin{figure}[H]
%%    \centering
...
%%\end{figure}
%%
%%\par Com as trajetórias identificadas, foi possível construir as distribuições angulares, na seção \ref{sec:sec_choque}.
