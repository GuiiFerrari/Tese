...
\par Com o alvo ativo é possível medir reações em diversos ângulos e energias. Portanto, é preciso primeiro discretizar (colocar em canais) os ângulos de espalhamento $\theta_p$ (ângulos de espalhamento dos prótons no referencial do laboratório). Cada canal $\theta_{bin}$ de $\theta_p$ possui largura de 4º. A seção de choque diferencial $\frac{\mathrm{d}\sigma}{\mathrm{d}\Omega} (\theta_p)$ é dada por \cite{zamora_mater}
...
\par A normalização deve ser obtida a partir da análise do canal elástico e inelástico do experimento. Até o momento da escrita dessa dissertação, ainda está sendo trabalhado o valor da normalização, portanto os valores apresentados das distribuições angulares estarão em unidades arbitrárias (u.a.).

\par A energia do feixe também foi discretizada. Como a energia do feixe depende da posição em $z$ do vértice de reação, ela foi discretizada a cada 10 mm, o que resulta em canais de 1.2 MeV de largura. A partir desses canais, foi calculada a energia do feixe. A figura \ref{fig:contagem_energia} mostra a contagem de eventos em função da energia do feixe. As distribuições angulares estão mostradas na figura \ref{fig:dist_ang}.
