...
\par Com o alvo ativo é possível medir reações em diversos ângulos e energias. Portanto, é preciso primeiro discretizar (colocar em canais) os ângulos de espalhamento $\theta_p$ (ângulos de espalhamento dos prótons). Cada canal $\theta_{bin}$ de $\theta_p$ possui largura de 4º. A seção de choque diferencial $\frac{\mathrm{d}\sigma}{\mathrm{d}\Omega} (\theta_p)$ é dada por

\begin{equation}
	\frac{\mathrm{d}\sigma}{\mathrm{d}\Omega} (\theta_p) = \frac{1}{\Delta\Omega(\theta_p) L},
\end{equation}

\par onde $L$ é a luminosidade (fator de normalização) e $\Delta\Omega(\theta_p)$ é o ângulo sólido calculado em $\theta_p$ dado por
...
\par A energia do feixe também foi discretizada. Como a energia do feixe depende da posição em $z$ do vértice de reação, ela foi discretizada a cada 10 mm, o que resulta em canais de 1.2 MeV de largura. A partir desses canais, foi calculada a energia do feixe. Algumas das distribuições angulares estão mostradas na figura \ref{fig:dist_ang}.
