...
\par Com o alvo ativo é possível medir reações em diversos ângulos e energias. Portanto, é preciso primeiro discretizar (colocar em bins) os ângulos de espalhamento $\theta_p$ (ângulos de espalhamento dos prótons no referencial do laboratório). Cada bin $\theta_{bin}$ de $\theta_p$ possui largura de 4º. A seção de choque diferencial $\frac{\mathrm{d}\sigma}{\mathrm{d}\Omega} (\theta_p)$ é dada por \cite{zamora_mater}
...
%    \end{description}
	\item $N_t$ \hfill \\
%	\begin{description}
		É o número total de partículas no alvo gasoso. Esse número é obtido a partir da equação dos gases ideais \cite{termo_mario}, onde chega-se em:
		\begin{equation}
			N_t = \frac{PdN_A}{RT},
		\end{equation}
		onde $P$ é a pressão do alvo gasoso, $d$ é o comprimento da câmara, $N_A$ é o número de Avogrado, $R$ a constante universal dos gases ideais e $T$ a temperatura.
%	\end{description}
	\item $N_b$ \hfill \\
%	\begin{description}
...
\par A normalização deve ser obtida a partir da análise do canal elástico do experimento. Até o momento da escrita dessa dissertação, ainda está sendo trabalhado o valor da normalização, portanto os valores apresentados das distribuições angulares estarão em unidades arbitrárias (u.a.).

\par A energia do feixe também foi discretizada. Como a energia do feixe depende da posição em $z$ do vértice de reação, ela foi discretizada a cada 10 mm, o que resulta em bins de 1.2 MeV de largura. A partir dessas bins, foi calculada a energia do feixe. A figura \ref{fig:contagem_energia} mostra a contagem de eventos em função da energia do feixe. As distribuições angulares estão mostradas na figura \ref{fig:dist_ang}.

\begin{figure}[H]
\centering
    \includegraphics[scale = 1., width=0.7\columnwidth]{figs/contagens_energia.png}
    \caption{Gráfico que mostra o número de eventos em função da energia do feixe $E_{beam}$. Os pontos de cor preta correspondem à contagem de eventos de breakup inclusivo e os pontos de cor vermelha correspondem à contagem de eventos de breakup exclusivo.}
...
\ContinuedFloat
    ...
    \end{subfigure}
\caption{Distribuições angulares de breakup do $^{17}$F em $^4$He em função do ângulo de espalhamento do próton $\theta_p$ no referencial do laboratório. A energia do feixe $E_{beam}$ está no referencial do centro de massa.}
\label{fig:dist_ang}
\end{figure}

\section{Análise e comparação dos resultados}

\par Os resultados mostrados na fig \ref{fig:dist_ang} mostram distribuições com energias acima (figuras \ref{subfig:dist_ang_a} - \ref{subfig:dist_ang_g}), próximas (figuras \ref{subfig:dist_ang_h} - \ref{subfig:dist_ang_l}) e abaixo (figuras \ref{subfig:dist_ang_m} - \ref{subfig:dist_ang_ab}) da barreira de Coulomb da reação (barreira de Coulomb é de 4.2 MeV). As distribuições próximas da barreira se distribuem mais igualmente em ângulos entre 20º e 50º. Distribuições acima da barreira possuem valores maiores em ângulos mais dianteiros (de 20º à 30º). Embaixo da barreira as distribuições são mais estreitas e parecem ter um valor máximo em 30º.

\par Para analisar reações de breakup (dissociação de Coulomb), usualmente se usa teoria de perturbação em primeira ordem, que demonstra resultados satisfatórios para os casos de breakup de núcleos ricos em nêutrons fracamente ligados. Para o breakup de núcleos ricos em prótons fracamente ligados, como o caso do $^{17}$F, são sugeridas correções de ordens superiores \cite{LIANG200922, BERTULANI2003199}.

\par Experimentos para o estudo do breakup do $^{17}$F em $^{58}$Ni e em $^{208}$Pb acima e próximos da barreira de Coulomb (energia da barreira de 39.13 MeV para a reação $^{17}$F + $^{58}$Ni e 88.03 Mev para a reação $^{17}$F + $^{208}$Pb) foram feitos e foram usados como comparação dos resultados.
...
\par Em comparação com os resultados mostrados na figura \ref{fig:dist_ang}, as distribuições com energias acima da barreira de Coulomb são semelhantes ao da figura \ref{subfig:dist_ang_2_a}, onde há a predominância de pontos com ângulos dianteiros. Para energias próximas da barreira de Coulomb, a distribuição apresentada na figura \ref{subfig:dist_ang_2_b} é mais semelhante, onde não há um crescimento grande para ângulos entre 5º e 10º. Importante notar que mesmo para os resultados com energias maiores que aproximadamente 1.64 MeV (ver figura \ref{subfig:dist_ang_w}) a seção de choque do breakup exclusivo é, pelo menos, da mesma ordem de grandeza para ângulos entre 20º e 40º, resultado interessante pois estão sendo comparados núcleos com valores de $Z$ muito diferentes.

\par Outro experimento do breakup do $^{17}$F em $^{58}$Ni com energias próximas da barreira de Coulomb também indica que correções de ordem superiores são necessárias para a análise das seções de choque \cite{YANG2021136045, Kolata2016}. Isso foi realizado com cálculos de CDCC (Continuum-Discretized Coupled-Channels), onde são acrescentados estados do contínuo do $^{17}$F.
